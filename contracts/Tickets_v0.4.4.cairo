...
// #############################################################################
    ...
    func burn{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(tokenId: Uint256) {
        ...
        let (market) = zklend_market_addrs.read();
        // let (eth_address) = ETH_contract_addrs.read();
        let (USDC_address) = USDC_contract_addrs.read();
        let (initial_deposit) = ticketPrice();
        let (deposit_felt) = uint256_to_felt(initial_deposit);

        IzkLendMarket.withdraw(contract_address=market, token=USDC_address, amount=deposit_felt);
