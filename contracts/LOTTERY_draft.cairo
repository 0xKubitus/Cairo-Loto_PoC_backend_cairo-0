...
func runLotteryDraw{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() {
    ...
    // let seed = hash(request_address, hash(nonce, block_timestamp));
    // ASK PRAGMA TEAM EXACTLY WHICH NONCE AM I SUPPOSED TO PROVIDE HERE
    // AS WELL AS HOW TO USE 'hash' FUNCTION BECAUSE I CAN ONLY FIND A 'hash2'
    // FUNCTION IN THE STARKNET LIBRARY NAMED 'hash.cairo' 
    // (cf: https://github.com/starkware-libs/cairo-lang/blob/master/src/starkware/cairo/common/hash.cairo)
...
func requestRandomNumber{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    ...
    // TODO: Write in Storage the request_id (probably bad practice but it seems it can't be returned from an invoke/external function)
